...
;(function($, window, undefined) {
    ...
    /**
	 * Are we under the breakpoint?
	 */
    var pageBreakpoint = 1024
      , underBreakpoint = $(window).width() <= pageBreakpoint;
    $(window).on("resize", jQuery.debounce(250, false, function(event) {
        underBreakpoint = $(window).width() <= pageBreakpoint;
    }));

    /**
	 * Animate the opening/closing of pallets in the mobile view
	 */
    $(window).on("load resize", function(event) {
        if ($(window).width() <= pageBreakpoint) {
            $("body").addClass("page__animation");
        } else {
            $("body").removeClass("page__animation");
        }
    });

    /**
	 * Prevent page scrolling when in mobile view, with an open pallet
	 */
    $('input:checkbox.page__toggle').change(function() {
        if ($(this).is(":checked")) {
    ...
    /**
	 * Add "is-fixed" class to .pallet__top when scrolling
	 */
    $(window).on('load scroll', function() {
        if ($(window).scrollTop() > 0) {
            $('#page__pallet__top').addClass('is-fixed');
            $('body').addClass('is-scrolling');
        } else {
            $('#page__pallet__top').removeClass('is-fixed');
    ...
    /**
	 * R2D2
	 * Ridiculously Responsive Drop-down Navigation
	 */
    var menu = $('ul.menu')
      , has_submenu = menu.find('li').has('ul');

    if (isTouchDevice()) {

        menu.toggleClass('mouseable touchable');

        $(has_submenu).children('a, span').on('touchstart click', function(e) {
            var link = $(this)
              , list = $(this).closest('li')
              , list_parents = list.parents('li')
              , list_submenu = list.children('ul');
            ...
            if (!underBreakpoint) {
                if (list.hasClass('hover')) {
                    ...
                    list.addClass('hover');
                    $('.hover').not(list).not(list_parents).removeClass('hover');
                    e.preventDefault();

                    var offset_l = list_submenu.offset().left;
                    if ($(window).width() - offset_l < list_submenu.width()) {
                        list_submenu.addClass('position-right');
                    }
    ...
    $(document).on('touchstart click', 'li.tb', function(e) {
        e.stopPropagation();

        if (!evnt) {
            // prevent touchstart and click events firing simultaneously
            evnt = true;
            setTimeout(function() {
                evnt = false;
            }, 100);
            ...
            var bubble = $(this).children('ul').first()
              , offset = bubble.offset().left;
            if (offset + bubble.width() > $(window).width()) {
                $(bubble).css('margin-left', (12 + bubble.width() - ($(window).width() - offset)) * -1);
            }
        }

    });

    $(document).on('touchstart click', 'li.tb ul', function(e) {
        e.stopPropagation();
    });

    /**
	 * Destroys Drop-menus and Toolboxes
	 */
    var destroyMenu = function(resize) {
        $('.hover').removeClass('hover');
        if (resize) {
            $('.position-right').removeClass('position-right');
        }
    }
    $(has_submenu).find('a').on('touchstart click', function(e) {
        e.stopPropagation();
        // on menu touch, prevent next $(document).on('touchstart'...
    });
    ...
    /**
	 * Setup Labels (th) for Table Cells (td)
	 * src: http://sitesforprofit.com/responsive-tables-in-wordpress
	 */
    var headertext = [];
	 ...
	 */
    var tableSetup = function(table) {
        var tableBreakpoint = parseFloat(table.attr('data-breakpoint'));
        if (typeof tableBreakpoint !== typeof undefined && tableBreakpoint !== false && !isNaN(tableBreakpoint)) {
            tableBreakpoint = tableBreakpoint;
        } else {
            tableBreakpoint = pageBreakpoint;
        }
        var stackTable = $(window).width() <= tableBreakpoint;

        if (stackTable) {
            table.addClass('stack-table');
        } else {
            table.removeClass('stack-table');
        }
    }
	 ...
	 * radioTabs Helper
	 */
    $('.radioTabs-js').each(function() {

        var el = $(this)
          , id = el.find('input[type=radio]:checked').attr('id');
        $('[data-for*=' + id + ']').addClass('is_open');
        ...
        el.on('change', 'input[type=radio]', function() {
            ...
            if (this.checked) {
                ...
                $('[data-for*=' + id + ']').addClass("is_open");
                $('.is_checked, .is_open').not(this).not('[data-for*=' + id + ']').removeClass("is_checked is_open");
            }
        });
    ...
    $('[data-fancybox=search]').fancybox({
        ...
        onInit: function(instance) {
            $(document).on('keypress.fb', function(e) {
    ...
    if ($('.search-form input').length && $('.search-handoff input').length) {
        $('.search-form input').keyup(function() {
            var value = $(this).val();
            $('.search-handoff input[name=q]').add('.search-handoff input[name=s]').val(value);
        }).keyup();
    }
    /**/

    /**
	 * Post notices to the top of the content area
	 */
    var postNotice = function(style, html) {
        var markup = $("<div/>").attr({
            class: "content " + style
        });
        markup = markup.html(html);
        var anchor = $(".main__column > .content"), notice;
        if ($(anchor).find("#notice").length > 0) {
            notice = $("#notice");
        } else {
            notice = $("<div/>").attr({
                class: "notice",
        ...
        notice.append(markup);
    }
    //postNotice('green', '<p><strong>Command, we have a problem</strong>.</p><p>There\'s a tribble in the food replicator.</p>');
    //postNotice('yellow', '<p><strong>Command, we have a problem</strong>.</p><p>There\'s a tribble in the food replicator.</p>');
    //postNotice('red', '<p><strong>Command, we have a problem</strong>.</p><p>There\'s a tribble in the food replicator.</p>');
    //postNotice('blue', '<p><strong>Command, we have a problem</strong>.</p><p>There\'s a tribble in the food replicator.</p>');

}(jQuery, window));

;(function($, window, undefined) {

    // Helps to prevent FOUC
    $('html').addClass('js');
