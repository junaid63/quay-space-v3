...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (dt_ajax_table) {
            new DataTable(dt_ajax_table, {
                ...
                scrollX:true,
                ajax: {
                    url: "{{ route('dashboard.activity.json') }}",
                    dataSrc: 'data'
                },
                order: [[0, 'desc']],
                columns: [
    ...
    $(function () {
        ...
        function resetModal() {
            ...
            $('#price').val('');
            $('#activity_id').val('');
            $('#modalCenterTitle').text('Add Activity');
        ...
        $(document).on('click', '.edit-btn', function () {
            ...
            const fetchUrl = `{{ route('dashboard.activity.show', ':id') }}`.replace(':id', id);

            $.get(fetchUrl, function (data) {
                $('#activity_id').val(data.id);
                $('#title').val(data.title);
                $('#price').val(data.price);
                $('#modalCenterTitle').text('Edit activity');
        ...
        $(document).on('click', '.delete-btn', function () {
            ...
            const url  = `{{ route('dashboard.activity.destroy', ':id') }}`.replace(':id', id);
        ...
        $('#saveFundingBtn').on('click', function () {
            ...
            const url = id
                ? `{{ route('dashboard.activity.update', ':id') }}`.replace(':id', id)
                : `{{ route('dashboard.activity.store') }}`;
    ...
    $(document).on('change', '.status-toggle', function () {
        ...
        $.ajax({
            url: '{{ route('dashboard.activity.toggleStatus', [':id']) }}'.replace(':id', id),
            method: 'PATCH',
            data: {
                _token: '{{ csrf_token() }}',
                status: status
...
<div class="container-fluid flex-grow-1 container-p-y">
    <div class="card">
        <div class="d-flex align-items-center justify-content-between p-4 pb-0">
            <div class="heading">
                <h5 class="card-header p-0 text-md-start text-center">Activity</h5>
            </div>
            <div class="button">
                <button type="button" class="btn btn-label-secondary waves-effect btn-add"><i class = "icon-base ti tabler-plus"></i> Add Activity</button>
            </div>
        </div>
