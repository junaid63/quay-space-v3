...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (dt_ajax_table) {
            new DataTable(dt_ajax_table, {
                ...
                scrollX:true,
                ajax: {
                    url: "{{ route('dashboard.spacetype.json') }}",
                    dataSrc: 'data'
                },
                order: [[0, 'desc']],
                columns: [
    ...
    $(function () {
        ...
        $(document).on('click', '.edit-btn', function () {
            ...
            const fetchUrl = `{{ route('dashboard.spacetype.show', ':id') }}`.replace(':id', id);

            $.get(fetchUrl, function (data) {
                $('#spacetype_id').val(data.id);
                $('#name').val(data.name);
                $('#description').val(data.description);
                $('#status').val(data.status);
                $('#modalCenterTitle').text('Edit Space Type');
        ...
        $(document).on('click', '.delete-btn', function () {
            ...
            const url  = `{{ route('dashboard.spacetype.destroy', ':id') }}`.replace(':id', id);
        ...
        $('#saveFundingBtn').on('click', function () {
            ...
            const url = id
                ? `{{ route('dashboard.spacetype.update', ':id') }}`.replace(':id', id)
                : `{{ route('dashboard.spacetype.store') }}`;
    ...
    $(document).on('change', '.status-toggle', function () {
        ...
        $.ajax({
            url: '{{ route('dashboard.spacetype.toggleStatus', [':id']) }}'.replace(':id', id),
            method: 'PATCH',
            data: {
                _token: '{{ csrf_token() }}',
                status: status
...
<div class="container-fluid flex-grow-1 container-p-y">
    <div class="card">
        <div class="d-flex align-items-center justify-content-between p-4 pb-0">
            <div class="heading">
                <h5 class="card-header p-0 text-md-start text-center">Spacetypes</h5>
            </div>
            <div class="button">
                <button type="button" class="btn btn-label-secondary waves-effect btn-add"><i class = "icon-base ti tabler-plus"></i> Add Type</button>
            </div>
        </div>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <input type="hidden" id="spacetype_id">
                <h5 class="modal-title" id="modalCenterTitle">Add Space Type</h5>
