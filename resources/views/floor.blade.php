...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (dt_ajax_table) {
            new DataTable(dt_ajax_table, {
                ...
                scrollX:true,
                ajax: {
                    url: "{{ route('dashboard.floor.json') }}",
                    dataSrc: 'data'
                },
                order: [[0, 'desc']],
                columns: [
    ...
    $(function () {
        ...
        function resetModal() {
            ...
            $('#status').val('');
            $('#floor_id').val('');
            $('#modalCenterTitle').text('Add Floor');
        ...
        $(document).on('click', '.edit-btn', function () {
            ...
            const fetchUrl = `{{ route('dashboard.floor.show', ':id') }}`.replace(':id', id);
        ...
        $(document).on('click', '.delete-btn', function () {
            ...
            const url  = `{{ route('dashboard.floor.destroy', ':id') }}`.replace(':id', id);
        ...
        $('#saveFundingBtn').on('click', function () {
            ...
            const url = id
                ? `{{ route('dashboard.floor.update', ':id') }}`.replace(':id', id)
                : `{{ route('dashboard.floor.store') }}`;
    ...
    $(document).on('change', '.status-toggle', function () {
        ...
        $.ajax({
            url: '{{ route('dashboard.floor.toggleStatus', [':id']) }}'.replace(':id', id),
            method: 'PATCH',
            data: {
                _token: '{{ csrf_token() }}',
                status: status
...
<div class="container-fluid flex-grow-1 container-p-y">
    <div class="card">
        <div class="d-flex align-items-center justify-content-between p-4 pb-0">
            <div class="heading">
                <h5 class="card-header p-0 text-md-start text-center">Floors</h5>
            </div>
            <div class="button">
                <button type="button" class="btn btn-label-secondary waves-effect btn-add"><i class = "icon-base ti tabler-plus"></i> Add Floor</button>
            </div>
        </div>
            ...
            <div class="modal-body">
                ...
                <div class="row">
                    <div class="col mb-4">
                        ...
                        <input type="number" id="number" class="form-control" placeholder="Enter Floor Number">
                    </div>
                </div>
