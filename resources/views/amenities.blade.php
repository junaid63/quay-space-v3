...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (dt_ajax_table) {
            new DataTable(dt_ajax_table, {
                ...
                scrollX:true,
                ajax: {
                    url: "{{ route('dashboard.amenities.json') }}",
                    dataSrc: 'data'
                },
                order: [[0, 'desc']],
                columns: [
    ...
    $(function () {
        ...
        function resetModal() {
            ...
            $('#status').val('');
            $('#spacetype_id').val('');
            $('#modalCenterTitle').text('Add Amenitiestype');
        ...
        $(document).on('click', '.edit-btn', function () {
            ...
            const fetchUrl = `{{ route('dashboard.amenities.show', ':id') }}`.replace(':id', id);

            $.get(fetchUrl, function (data) {
                $('#spacetype_id').val(data.id);
                $('#name').val(data.name);
                $('#status').val(data.status);
                $('#modalCenterTitle').text('Edit Amenities');
        ...
        $(document).on('click', '.delete-btn', function () {
            ...
            const url  = `{{ route('dashboard.amenities.destroy', ':id') }}`.replace(':id', id);
        ...
        $('#saveFundingBtn').on('click', function () {
            ...
            const url = id
                ? `{{ route('dashboard.amenities.update', ':id') }}`.replace(':id', id)
                : `{{ route('dashboard.amenities.store') }}`;
    ...
    $(document).on('change', '.status-toggle', function () {
        ...
        $.ajax({
            url: '{{ route('dashboard.amenities.toggleStatus', [':id']) }}'.replace(':id', id),
            method: 'PATCH',
            data: {
                _token: '{{ csrf_token() }}',
                status: status
...
<div class="container-fluid flex-grow-1 container-p-y">
    <div class="card">
        <div class="d-flex align-items-center justify-content-between p-4 pb-0">
            <div class="heading">
                <h5 class="card-header p-0 text-md-start text-center">Amenities</h5>
            </div>
            <div class="button">
                <button type="button" class="btn btn-label-secondary waves-effect btn-add"><i class = "icon-base ti tabler-plus"></i> Add Amenity</button>
            </div>
        </div>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h5 class="modal-title" id="modalCenterTitle">Add Amenities</h5>
